...
class RevolunaSupabaseUser extends BaseAuthUser {
  RevolunaSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> revolunaSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = RevolunaSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
